...
static int event_msix_fd_reg(Dpdk_Context_t *dpdkc, DpdkInstance *dpdk_ins)
{
    ...
    for (queueid = 0; queueid < dpdk_ins->n_rx_queue; ++queueid) {
        ...
        ret = rte_eth_dev_rx_intr_ctl_q(portid, queueid,
                ...
                (void *)((uintptr_t)data));
        if (ret) {
            DAQ_RTE_LOG("%s: dev rx intr_ctl for queue %d failed\n", __func__, queueid);
            return ret;
        }
...
static inline void turn_on_intr(DpdkInstance *dpdk_ins)
{
    ...
    for (queue_id = dpdk_ins->rx_queue_s; queue_id < dpdk_ins->rx_queue_e; ++queue_id) {
        ...
        DAQ_RTE_LOG_DEEP("%s: port %d, queue %d, enable irq(ret-%d)\n",
        		__func__, port_id, queue_id, ret);
    }
}
    ...
    for (i = 0; i < n; i++) {
        if ( dpdk_ins->sigfd == event[i].fd ) {
            ...
            if ( read_sz != sizeof(sig_info) ) {
                DAQ_RTE_LOG_DEEP("%s: lcore %u is waked up from invalid signal interrupt\n",
                        __func__, rte_lcore_id());
                break;
            }
            ...
            case SIGTERM:
                ...
                DAQ_RTE_LOG_DEEP("%s: lcore %u is waked up from signal interrupt\n",
                        __func__, rte_lcore_id());
                break;
            case SIGCONT:
                DAQ_RTE_LOG_DEEP("%s: lcore %u is waked up from SIGCONT signal and continue\n",
                        __func__, rte_lcore_id());
                break;
            default:
                DAQ_RTE_LOG_DEEP("%s: lcore %u is waked up from other signal, continue\n",
                        __func__, rte_lcore_id());
                break;
            }
            break;
        ...
        DAQ_RTE_LOG_DEEP("%s: lcore %u is waked up from rx interrupt on"
            " port %d queue %d, disable irq(ret-%d)\n",
            __func__, rte_lcore_id(), port_id, queue_id, ret);
        break;
    }
